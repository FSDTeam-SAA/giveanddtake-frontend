...
async function postJob(data: any, retries = 2): Promise<any> {
  ...
  try {
    ...
    console.log("[v0] Job post response headers:", Object.fromEntries(response.headers.entries()))
...
export default function MultiStepJobForm() {
  ...
  const handleStepClick = async (targetStep: number) => {
    ...
    if (targetStep <= currentStep) {
      console.log("[v0] Navigating to previous/current step without validation")
  ...
  const handlePublish = async (data: JobFormData) => {
    ...
    try {
      ...
      const postData = {
        ...
        customQuestion:
          data.customQuestions?.filter((q) => q.question).map((q) => ({ question: DOMPurify.sanitize(q.question!) })) ||
          [],
      ...
      <div className="container mx-auto px-4">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handlePublish)} className="space-y-6">
            ...
            {currentStep === 4 && <CustomQuestionsStep form={form} onNext={handleNext} onCancel={handleCancel} />}
