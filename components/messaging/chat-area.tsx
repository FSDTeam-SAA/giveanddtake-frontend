...
export function ChatArea({ roomId, userId, socket, onBackToList, roomName }: ChatAreaProps) {
    ...
    const {
        ...
        queryFn: async ({ pageParam = 1 }) => {
            ...
            return data.success ? data : { data: [], meta: { page: 1, totalPages: 1 } }
        },
    ...
    const allMessages = data?.pages.flatMap((page) => (page as { data: Message[] }).data).reverse() || []
            ...
            >
                ...
                {allMessages.map((msg: Message, index) => (
                    <MessageBubble
                        ...
                        showAvatar={
                            index === 0 ||
                            allMessages[index - 1]?.userId !== msg.userId
                        }
                    />
                ))}
