...
export function MessageBubble({ message, isOwn, showAvatar }: MessageBubbleProps) {
  ...
  const handleEdit = () => {
    if (editText.trim() !== message.message) {
      updateMessageMutation.mutate({ messageId: message._id, newMessage: editText.trim() });
    } else {
      setIsEditing(false);
    }
  };
  ...
  const renderFilePreview = (file: { filename: string; url: string }) => {
    ...
    if (["mp4", "mov", "avi", "webm", "mkv"].includes(ext || "")) {
      return <video src={file.url} controls className="max-w-[200px] sm:max-w-xs max-h-48 rounded-lg" />;
    }
    return (
      <a
        href={file.url}
  ...
  const perMessageAvatar =
    typeof message.userId === "string" ? undefined : message.userId?.avatar?.url;

  return (
    <div className={cn("flex", isOwn ? "justify-end" : "justify-start")}>
      <div className={cn("flex max-w-[85%] sm:max-w-xs lg:max-w-md", isOwn ? "flex-row-reverse" : "flex-row")}>
        {showAvatar && !isOwn && (
          <Avatar className="w-8 h-8 mt-1 flex-shrink-0">
            <AvatarImage
              src={perMessageAvatar || "/placeholder.svg?height=32&width=32&text=U"}
            />
            <AvatarFallback>U</AvatarFallback>
          </Avatar>
        )}

        <div className={cn("mx-2", showAvatar && !isOwn && "ml-2", showAvatar && isOwn && "mr-2")}>
            ...
            >
              {isEditing ? (
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <Button size="sm" variant="ghost" onClick={handleEdit} disabled={updateMessageMutation.isPending}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="absolute -left-6 md:-left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1"
                  >
