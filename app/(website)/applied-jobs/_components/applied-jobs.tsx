...
export default function AppliedJobs() {
  ...
  const { data: appliedJobs, isPending, isError, error } = useQuery({
    queryKey: ["appliedJobs", userId],
    queryFn: () => getAppliedJobs(userId),
    enabled: !!userId,
  });

  console.log("My applied jobs are: ", appliedJobs?.data?.applications)
        ...
        appliedJobs?.data?.applications?.map((job: any) => (
          <div key={job._id} className="bg-[#F8F8F8] lg:p-7 p-3 lg:space-y-5 space-y-2 rounded-md">
            <div className="flex justify-between items-center">
              <div className="">
                <h3 className="lg:text-lg text-base font-semibold">{job.jobId.title}</h3>
              </div>
