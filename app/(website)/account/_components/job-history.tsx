...
const fetchJobHistory = async (userId: string, page: number): Promise<JobHistoryResponse> => {
...
export default function JobHistory() {
                ...
                <TableBody>
                  {Array.from({ length: 5 }).map(
                    (
                      ...
                      <TableRow key={index} className={cn(index % 2 === 1 ? "bg-gray-50" : "")}>
  ...
  if (isError) {
    return <div className="flex justify-center items-center h-screen text-red-500">Error loading job history.</div>
  }

  if (!session || !userId) {
    return <div className="flex justify-center items-center h-screen">Please log in to view your job history.</div>
  }

  const applications = data?.data?.applications || []
  const meta = data?.meta || { currentPage: 1, totalPages: 0, totalItems: 0, itemsPerPage: 10 }
      ...
      <Card>
        ...
        <CardContent>
          {applications.length === 0 ? (
            <div className="text-center text-muted-foreground py-8">No job applications found.</div>
          ) : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {applications.map((application, index) => (
                    <TableRow key={application._id} className={cn(index % 2 === 1 ? "bg-gray-50" : "")}>
                      <TableCell className="font-medium">{application.jobId.title}</TableCell>
                      <TableCell>Arrex Digital</TableCell> {/* Hardcoded as per image, no company name in API */}
                      <TableCell>
                        {new Date(application.createdAt).toLocaleDateString("en-US", { month: "long", day: "numeric" })}
                      </TableCell>
                      <TableCell
                        className={cn(
                          "font-medium",
